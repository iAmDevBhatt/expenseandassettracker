...
- `AppShell.tsx` — outer layout with `<Outlet />`; renders `<InstallBanner />` above every page
...
- **iOS**: `index.html` sets `apple-mobile-web-app-*` metas and `viewport-fit=cover`; safe-area insets handled by the `pt-safe` / `pb-safe` / `bottom-safe` / `sheet-footer` utilities in `index.css`
- **Install**: in-app banner + ☰ menu item (Chromium), Share → Add to Home Screen (iOS Safari). **Requires HTTPS** (or `localhost`); over plain `http://<LAN-IP>` the service worker never registers
