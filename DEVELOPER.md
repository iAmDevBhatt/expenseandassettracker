...
- `AppShell.tsx` — outer layout with `<Outlet />`; renders `<InstallBanner />` above every page and `<BottomNav />` below it
- `InstallBanner.tsx` — dismissible "Install Tracker on your phone" prompt (native install button on Chromium, Share → Add to Home Screen hint on iOS); hidden once installed/dismissed (dismissal kept in `localStorage`)
- `Navbar.tsx` — top bar; desktop (`lg:`) shows all nav links inline plus username/sign-out, order: Graphs → Budget → Assets → Loans → Expenses → Users → Configuration; below `lg` it's just the logo/app name (navigation moves to `BottomNav`)
- `BottomNav.tsx` — fixed icon tab bar for phones/tablets (`lg:hidden`): Expenses, Budget, Assets, Loans, Graphs, plus a **More** tab that opens a bottom-sheet `Modal` with Users, Configuration, Install app, username, and Sign out
- `icons.tsx` — small inline SVG icons used by `BottomNav` (no icon-library dependency)

### `src/components/common/`
- `CurrencyCell.tsx` — formats number as ₹X,XX,XXX.XX using `Intl.NumberFormat('en-IN')`
- `Modal.tsx` — accessible dialog (Escape / backdrop click to close); centered on `sm+`, bottom sheet with grab handle on phones; locks body scroll while open
- `LoadingSpinner.tsx` — spinning indicator with label

### `src/components/expense/`
- `ExpenseTable.tsx` — expenses with add/edit/delete; table on `sm+`, day-grouped tap-to-edit list + floating **+** button on phones; header shows entry count and month total; computes the "quick categories" (most-used this month, topped up from config order); `?add=1` in the URL opens the add sheet (PWA shortcut)
- `ExpenseForm.tsx` — shared add/edit form: amount first (`inputMode="decimal"`), category chips + searchable list, Today/Yesterday date chips, payment-method chips, sticky footer. Labels come from `addexpense.*` / `editexpense.*` via `labelPrefix`, new strings from `expenseform.*`
- `AddExpenseModal.tsx` — wraps `ExpenseForm`; supports **Save & add another** (keeps date + payment method, clears the rest). Default date = today if viewing the current month, else the 1st of the viewed month
- `EditExpenseModal.tsx` — wraps `ExpenseForm`; includes a Delete button
...
### `src/hooks/`
- `useLabels.ts` — loads `labels.properties`, returns `l(key, fallback?)`
- `useInstallPrompt.ts` — captures `beforeinstallprompt` at module load; exposes `canInstall`, `promptInstall()`, `showIOSHint`, `isStandalone`
...
3. Add nav link in `src/components/layout/Navbar.tsx` (desktop); if it's a primary section, also add a tab (with an icon from `icons.tsx`) to `TABS` in `BottomNav.tsx` — otherwise it's reachable from the **More** sheet
...
- **Caching strategy**: NetworkFirst for `/api/**` (fresh data + offline fallback); `navigateFallbackDenylist` keeps `/api`, `/docs`, `/redoc`, `/openapi.json`, `/health` out of the SPA fallback
- **Manifest extras**: `id`, `scope`, and an **Add expense** shortcut → `/expenses?add=1`
- **iOS**: `index.html` sets `apple-mobile-web-app-*` metas and `viewport-fit=cover`; safe-area insets handled by the `pt-safe` / `pb-safe` / `bottom-safe` / `bottom-safe-nav` / `sheet-footer` utilities in `index.css`
- **Install**: in-app banner + **More** tab item in `BottomNav` (Chromium), Share → Add to Home Screen (iOS Safari). **Requires HTTPS** (or `localhost`); over plain `http://<LAN-IP>` the service worker never registers
- **Serving in Docker**: `vite-plugin-pwa` writes `sw.js`, `registerSW.js`, `manifest.webmanifest`, `workbox-*.js` to the **root** of `dist/`. The `SERVE_STATIC` catch-all in `main.py` serves any real file under `dist/` (path-traversal safe) before falling back to `index.html`; `sw.js`, `registerSW.js`, `manifest.webmanifest` and `index.html` are sent with `Cache-Control: no-cache` so new deploys are picked up
- **Updates**: `registerType: 'autoUpdate'`, so the first load after a deploy may still show the old build and the next load shows the new one
